...
app.factory('AuthService', function ($http) {
    var authService = {};
    authService.login = function (para) {
        ...
        //    return data;
        //});
        return $http.get('https://api.appery.io/rest/1/apiexpress/api/ocsapicall/spmOCS_LiveOCSLoginPowerBI', { params: JSON.parse(para) }).then(function (response) {
            console.log(response);
            var data = {};
            var res = response.data;
            if (res[0].hasOwnProperty("ServerMsg")) {
                if (res[0].ServerMsg == 'Invalid Login Credentials') {
                    data.status = false;
                    data.message = res[0].ServerMsg;
                    data.data = null;
